...
import { reviewedBuffBriefFor } from './reviewed-buff-briefs.mjs';
...
export function transform({ versions, enemies, buffs, now = new Date(), commit = pinned }) {
  ...
  const cycleBuffs = version.versionBuffIDs.map(id => { const buff = buffs[id]; if (!buff) throw new Error(`missing buff ID ${id}`); const description = stripHtml(buff[2] || buff[1]); return { id, name: buff[0], description, segments: segmentDescription(description), ...reviewedBuffBriefFor(id, description) }; });
