...
import { cycleStatusFromData, validateCycleStatus } from '../cycle-status.mjs';
...
const cycleStatus = cycleStatusFromData(currentData);
const cycleStatusErrors = validateCycleStatus(cycleStatus);
if (cycleStatusErrors.length) throw new Error(`invalid cycle status: ${cycleStatusErrors.join('; ')}`);
fs.writeFileSync(path.join(dist, 'data/cycle-status.json'), JSON.stringify(cycleStatus));
const trendPins = [];
...
const emittedCycleStatus = compactJs(fs.readFileSync(path.join(root, 'cycle-status.mjs'), 'utf8'));
const cycleStatusHash = contentHash(emittedCycleStatus);
emittedJs = replaceAssetReference(emittedJs, './cycle-status.mjs', `./cycle-status.${cycleStatusHash}.mjs`, 'cycle status module');
...
fs.writeFileSync(path.join(dist, `cycle-status.${cycleStatusHash}.mjs`), emittedCycleStatus);
