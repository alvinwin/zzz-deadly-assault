...
test('desktop shows current cards, semantic chips, escaped content, sources, and footer feedback link', async ({ page }) => { await page.goto('/'); await expect(page.locator('.card')).toHaveCount(4); await expectShiyuLink(page); await expectCardsToBuffGap(page); expect(await page.locator('#cards').evaluate(element => element.compareDocumentPosition(document.querySelector('#global-buffs')) & Node.DOCUMENT_POSITION_FOLLOWING)).toBeTruthy(); await expect(page.locator('.mechanic-callout')).toHaveCount(4); await expect(page.locator('.mechanic-callout').first()).toContainText('Apply an Attribute Anomaly to add 1 Blight Mark for 30 seconds.'); await expect(page.locator('.specialty-chip')).toHaveCount(3); await expect(page.locator('.specialty-anomaly')).toContainText('Recommended: Anomaly'); await expect(page.locator('.specialty-stun')).toContainText('Recommended: Stun'); await expect(page.locator('.specialty-rupture')).toContainText('Recommended: Rupture'); await expect(page.locator('.specialty-fit small').nth(0)).toBeVisible(); await expect(page.locator('.specialty-fit small').nth(0)).toHaveText('Suitable for Agents with Anomaly specialty.'); await expect(page.locator('.specialty-fit small').nth(1)).toBeVisible(); await expect(page.locator('.specialty-fit small').nth(1)).toHaveText('Suitable for Agents with Stun specialty.'); await expect(page.locator('.specialty-fit small').nth(2)).toBeVisible(); await expect(page.locator('.specialty-fit small').nth(2)).toHaveText('Suitable for Agents with Rupture specialty.'); await expect(page.locator('.trend-up').first()).toBeVisible(); await expect(page.locator('.trend-flat').first()).toContainText('→ +0.0%'); await expect(page.locator('.trend-down')).toHaveCount(0); await expect(page.locator('.sparkline').first()).toHaveAttribute('role', 'img'); await expect(page.getByRole('img', {name:/3\.1\.1 HP 197200218.*3\.1\.2 HP 197200218/}).first()).toBeVisible(); await expect(page.locator('.card').nth(3)).toContainText('first appearance'); await expect(page.locator('.card').nth(3).locator('.sparkline')).toHaveCount(0); await expect(page.locator('.card').nth(0).locator('h3')).toHaveText('Girtablullu: Stagnant Aberrant'); await expect(page.locator('.card').nth(0).locator('h3 br')).toHaveCount(1); await expect(page.locator('.card').nth(3).locator('h3')).toHaveText('Rewritten: Sanguine Sweeper'); await expect(page.locator('.card').nth(3).locator('h3 br')).toHaveCount(1); await expect(page.locator('.card').nth(1).locator('h3 br')).toHaveCount(0); await expect(page.locator('.card').nth(2).locator('h3 br')).toHaveCount(0); await expectFreshnessStatus(page); await expect(page.locator('.section-head')).not.toContainText('cycle data for'); await expect(page.locator('dt').filter({hasText:'Weaknesses'}).first()).toBeVisible(); await expect(page.locator('.card').first()).toContainText('none'); await expect(page.locator('.card').nth(1).locator('.element-chip')).toHaveText(['Ice', 'Physical', 'Wind', 'Electric']); await expect(page.locator('.card').nth(1).locator('.element-chip').first()).toHaveClass(/element-ice/); await expect(page.locator('#global-buffs')).toContainText('Global buffs'); await expect(page.locator('#buff-list article')).toHaveCount(3); await expect(page.locator('#buff-list a')).toHaveCount(3); await expect(page.locator('#buff-list .text-annotation-quantity').first()).toBeVisible(); await expect(page.locator('#buff-list .text-annotation-effect-term').first()).toBeVisible(); await expect(page.locator('#buff-list .text-annotation-mechanic').first()).toBeVisible(); await expect(page.locator('#buff-list .text-annotation').filter({hasText:'Anomaly specialty'})).toContainText('Anomaly specialty'); await expect(page.locator('#buff-list .text-annotation').filter({hasText:'Attack specialty'})).toContainText('Attack specialty'); await expect(page.locator('#buff-list .text-annotation').filter({hasText:'Rupture specialty'})).toContainText('Rupture specialty'); await expect(page.locator('.provenance').first().locator('a')).toHaveCount(3); await expect(page.locator('.provenance a').first()).toHaveAttribute('href', /620a7546b4d2934c58d55cdf7a435056576bb1fc/); expect(await page.locator('#buff-list').evaluate(element => element.innerHTML)).not.toMatch(/<\/?(?:li|b)>/i); expect(await page.locator('#global-buffs').evaluate(element => getComputedStyle(element).contentVisibility)).toBe('visible'); expect(await page.locator('body')).not.toContainText('<script>'); let rect = await expectFeedbackLink(page); await expectNoActionOverlap(page, rect); await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight)); rect = await expectFeedbackLink(page); await expectNoActionOverlap(page, rect); await page.screenshot({path:'test-results/desktop.png', fullPage:true}); });
test('mechanic emphasis stays inline with adjacent prose and punctuation', async ({ page }) => { await page.goto('/'); const mechanics = await page.locator('.mechanic-callout p').evaluateAll(paragraphs => paragraphs.map(paragraph => ({ text: paragraph.textContent, emphasis: [...paragraph.querySelectorAll('.text-annotation')].map(element => ({ tag: element.tagName, display: getComputedStyle(element).display, parent: element.parentElement === paragraph })), blockDescendants: [...paragraph.querySelectorAll('*')].filter(element => getComputedStyle(element).display === 'block').length }))); expect(mechanics.some(({ text }) => text.includes('30 seconds. Each mark'))).toBe(true); expect(mechanics.flatMap(({ emphasis }) => emphasis)).toEqual(expect.arrayContaining([expect.objectContaining({ tag: 'STRONG', display: 'inline', parent: true }), expect.objectContaining({ tag: 'SPAN', display: 'inline', parent: true })])); expect(mechanics.every(({ blockDescendants }) => blockDescendants === 0)).toBe(true); });
test('adversity mechanic keeps compound combat terms intact', async ({ page }) => { await page.goto('/'); const mechanic = page.locator('.card.adversity .mechanic-callout'); const effectTerms = mechanic.locator('.text-annotation-effect-term'); await expect(effectTerms.filter({ hasText: /^PEN Ratio$/ })).toHaveCount(1); await expect(effectTerms.filter({ hasText: /^CRIT DMG$/ })).toHaveCount(1); await expect(effectTerms.filter({ hasText: /^DMG$/ })).toHaveCount(0); });
test('mobile cards keep their width and footer feedback link fits without horizontal overflow', async ({ page }) => { await page.setViewportSize({ width: 390, height: 844 }); await page.goto('/'); await expect(page.locator('.card')).toHaveCount(4); await expectShiyuLink(page); await expectCardsToBuffGap(page); await expectFreshnessStatus(page); expect(await page.locator('#buff-list').evaluate(element => getComputedStyle(element).rowGap)).toBe('24px'); const geometry = await page.locator('.trial-cards').evaluate(carousel => ({ carouselScrollWidth: carousel.scrollWidth, carouselClientWidth: carousel.clientWidth, firstCard: (() => { const rect = carousel.querySelector('.card').getBoundingClientRect(); const carouselRect = carousel.getBoundingClientRect(); return { x: rect.x, carouselX: carouselRect.x, width: rect.width }; })() })); expect(await page.evaluate(() => document.documentElement.scrollWidth <= document.documentElement.clientWidth)).toBe(true); expect(geometry.carouselScrollWidth).toBeGreaterThan(geometry.carouselClientWidth); expect(geometry.firstCard.width).toBeGreaterThan(320); expect(Math.round(geometry.firstCard.x)).toBe(Math.round(geometry.firstCard.carouselX)); const scrollLeftBefore = await page.locator('.trial-cards').evaluate(carousel => carousel.scrollLeft); await page.locator('.trial-cards').evaluate(carousel => { carousel.scrollLeft = carousel.scrollWidth; }); const scrollLeftAfter = await page.locator('.trial-cards').evaluate(carousel => carousel.scrollLeft); expect(scrollLeftAfter).toBeGreaterThan(scrollLeftBefore); let rect = await expectFeedbackLink(page); await expectNoActionOverlap(page, rect); await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight)); rect = await expectFeedbackLink(page); await expectNoActionOverlap(page, rect); expect(await page.evaluate(() => document.documentElement.scrollWidth <= document.documentElement.clientWidth)).toBe(true); await page.screenshot({path:'test-results/mobile.png', fullPage:true}); });
