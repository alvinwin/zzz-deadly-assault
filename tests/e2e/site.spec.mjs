...
test('desktop shows current cards, semantic chips, escaped content, sources, and floating feedback link', async ({ page }) => { await page.goto('/'); await expect(page.locator('.card')).toHaveCount(4); await expect(page.locator('#status')).toContainText('verified brief'); await expect(page.locator('#freshness')).toContainText('checked'); await expect(page.locator('dt').filter({hasText:'Weaknesses'}).first()).toBeVisible(); await expect(page.locator('.card').first()).toContainText('none'); await expect(page.locator('.card').nth(1).locator('.element-chip')).toHaveText(['Ice', 'Physical', 'Wind', 'Electric']); await expect(page.locator('.card').nth(1).locator('.element-chip').first()).toHaveClass(/element-ice/); await expect(page.locator('#global-buffs')).toContainText('GLOBAL BUFFS'); await expect(page.locator('#buff-list article')).toHaveCount(3); await expect(page.locator('#buff-list a')).toHaveCount(3); await expect(page.locator('#buff-list .emphasis-value').first()).toBeVisible(); await expect(page.locator('#buff-list .emphasis-damage').first()).toBeVisible(); await expect(page.locator('#buff-list .emphasis-mechanic').first()).toBeVisible(); await expect(page.locator('.provenance').first().locator('a')).toHaveCount(3); await expect(page.locator('.provenance a').first()).toHaveAttribute('href', /620a7546b4d2934c58d55cdf7a435056576bb1fc/); expect(await page.locator('#buff-list').evaluate(element => element.innerHTML)).not.toMatch(/<\/?(?:li|b)>/i); await expect(page.locator('body')).not.toContainText('<script>'); let rect = await expectFeedbackLink(page); await expectNoActionOverlap(page, rect); await page.evaluate(() => window.scrollTo(0, document.body.scrollHeight)); rect = await expectFeedbackLink(page); await expectNoActionOverlap(page, rect); await page.screenshot({path:'test-results/desktop.png', fullPage:true}); });
