...
test('transformation resolves cycle, affinities, buffs, and text annotations', () => { const source = { name: 'Boss', baseHP: [10], elementMult: [0.8, 1.2, 1, 1, 1, 1], desc: ['<li>Source sentence.</li>'], misc: '' }; const versionEnemies = ['e', 'x', 'y', 'z'].map(id => ({ id, type: 0 })); const result = transform({ now: new Date('2026-08-14T12:00:00Z'), versions: { x: { versionName: 'x', versionTime: '14/08/2026 - 28/08/2026', versionBuffIDs: ['b'], versionHPMult: [200, 200, 200, 200], versionEnemies } }, enemies: { e: source, x: source, y: source, z: source }, buffs: { b: ['Buff', '', '<li>Hello <b>30%</b> Ice</li>'] } }); assert.deepEqual(result.encounters[0].weaknesses, ['ice']); assert.deepEqual(result.encounters[0].resistances, ['fire']); assert.equal(result.buffs[0].description, 'Hello 30% Ice'); assert.deepEqual(result.buffs[0].segments, [[6, 9, 'quantity'], [10, 13, 'attribute']]); });
